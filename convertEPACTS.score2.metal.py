...
def main(inputfile, outputfile):
	...
	with open(outputfile,"w") as outfh:
		for line in infh:
			if line[0] == "#":
				outfh.write("MARKER\tREF_ALLELE\tEFFECT_ALLELE\tSAMPLESIZE\tEFFECT_FREQ\tP_VALUE\tSCORE\tDIRECTION\n")
			else:
				...
				outfh.write(testMarker+"\t"+otherREF+"\t"+otherEFFECT+"\t"+str(SAMPLESIZE)+"\t"+str(EFFECT_FREQ)+"\t"+PVALUE+"\t"+str(MAGNITUDE)+"\t"+DIRECTION+"\n")
	infh.close()
